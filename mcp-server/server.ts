...
server.registerTool(
  "qdrant_create_collection",
  {
    ...
    description:
      "Create a named vector collection. The 'size' must match the embedding model output dimension (nomic-embed-text = 768, llama3.2 = 3072).",
    inputSchema: {
      name: z.string().min(1),
...
server.registerTool(
  "rag_ingest",
  {
    ...
    description:
      "Split text into chunks, embed each chunk with Ollama, and upsert into a Qdrant collection. " +
...
server.registerTool(
  "rag_search",
  {
    ...
    description:
      "Embed a query with Ollama and return the top-K most similar chunks from a Qdrant collection.",
    inputSchema: {
      collection: z.string().min(1),
...
server.registerTool(
  "rag_ask",
  {
    ...
    description:
      "Embed the question, retrieve relevant chunks from Qdrant, then generate an answer using the local Ollama Llama model.",
    inputSchema: {
      collection: z.string().min(1),
