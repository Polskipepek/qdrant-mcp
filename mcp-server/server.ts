...
server.registerTool(
  ...
  async ({ name, size, distance }) => {
    try {
      ...
      return {
        content: [{ type: "text", text: JSON.stringify({ ok: result, name, size }, null, 2) }],
...
server.registerTool(
  ...
  async () => {
    try {
      ...
      return {
        content: [{ type: "text", text: repos.length ? repos.join("\n") : "No repos ingested yet." }],
...
server.registerTool(
  "rag_ingest",
  {
    ...
    inputSchema: {
      text: z.string().min(1),
      repo: z.string().min(1).describe("Repository or project name, e.g. 'bb-pay'"),
      source: z.string().min(1).describe("File path or URL the text came from"),
      language: z.string().optional().describe("Programming language or doc type, e.g. 'csharp', 'markdown'"),
  ...
  async ({ text, repo, source, language, branch, chunkSize = 500 }) => {
    try {
      ...
      return {
        content: [{ type: "text", text: `Ingested ${points.length} chunks from '${source}' (repo: ${repo}).` }],
...
server.registerTool(
  ...
  async ({ repo }) => {
    try {
      ...
      return {
        content: [{ type: "text", text: `Deleted all vectors for repo '${repo}'.` }],
...
server.registerTool(
  "rag_search",
  {
    ...
    inputSchema: {
      ...
      repo: z.string().optional().describe("Scope search to this repo only. Omit to search all repos."),
...
server.registerTool(
  "rag_ask",
  {
    ...
    inputSchema: {
      ...
      repo: z.string().optional().describe("Scope context retrieval to this repo. Omit to search all repos."),
      topK: z.number().int().positive().optional(),
    },
  },
  async ({ question, repo, topK = SEARCH_TOP_K }) => {
    try {
      ...
      if (results.length === 0) {
        return { content: [{ type: "text", text: "No relevant context found in the collection." }] };
      }
